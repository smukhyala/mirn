...
from mirn.viz.theme import DARK_PALETTE, apply_matplotlib
...
def null_distribution_figure(
    ...
    axis.hist(
        null_samples, bins=32, color=DARK_PALETTE.counterfactual, alpha=0.85, edgecolor="none"
    )
    axis.axvline(mdp_95, color=DARK_PALETTE.floor, linewidth=1.4, linestyle="--")
    axis.annotate(
...
def confounding_sweep_figure(sweep_frame: pd.DataFrame) -> Figure:
    ...
    axis.axhspan(0.0, 1.0, color=DARK_PALETTE.floor, alpha=0.22, linewidth=0.0)
    axis.axhline(1.0, color=DARK_PALETTE.floor, linewidth=1.2, linestyle="--")
    axis.annotate(
        "detection floor",
        xy=(0.0, 1.0),
        xytext=(6.0, 8.0),
    ...
    axis.fill_between(
        ...
        reported_low_mdp,
        reported_high_mdp,
        color=DARK_PALETTE.naive,
        alpha=0.18,
    )
    axis.plot(axis_values, reported_mdp, color=DARK_PALETTE.naive, linewidth=1.8, label="reported")
    axis.plot(
        axis_values,
        true_values_mdp,
        color=DARK_PALETTE.paired,
        linewidth=1.8,
        linestyle="--",
        label="true (paired)",
    )
